...
Three weights of Noto Sans Devanagari — Regular, SemiBold and Bold — are
compiled into the binary the same way and for the same reason as Inter, and are
used to draw a caption Inter has no glyphs for (D-157). Inter covers Latin,
Greek and Cyrillic; a Hindi caption used to render as one `.notdef` box per
character, so a line of Hindi came out as a row of little NO GLYPH labels.

The same condition 2 applies, so a copy of spoonstill contains this:
